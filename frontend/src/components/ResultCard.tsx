import { CheckCircle2, Lightbulb, Activity, RotateCcw } from "lucide-react";
...
export function ResultCard({ result, onReset }: { result: PredictionResult; onReset: () => void }) {
    ...
    <Card className="p-8 shadow-elegant animate-fade-in border-2 border-primary/10">
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <h2 className={`text-4xl font-bold tracking-tight ${isHealthy ? "text-primary" : "text-gradient"}`}>
            {result.disease}
          </h2>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-gradient">{result.confidence.toFixed(1)}%</span>
          </div>
